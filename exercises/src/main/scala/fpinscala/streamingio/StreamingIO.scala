...
input (`Await`) or signaling termination via `Halt`.
  ...
  object Process {
    ...
    def count[I]: Process[I, Int] = {
      def go(count: Int): Process[I, Int] = await(_ => emit(count, go(count + 1)))
    ...
    def count3[I]: Process[I, Int] = loop(1)((_, s) => (s, s + 1))
    ...
    def zip[A, B, C](p1: Process[A, B], p2: Process[A, C]): Process[A, (B, C)] =
      (p1, p2) match {
        ...
        case (Await(f), _) => Await(a => zip(f(a), feed(a)(p2)))
        case (_, Await(f)) => Await(a => zip(feed(a)(p1), f(a)))
      }
